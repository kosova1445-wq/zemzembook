...
function injectViews(){
 ...
 <section id="view-pwa" class="view"><div class="growth-grid">
  <article class="growth-card"><h3>PWA / Install App</h3><p>Lejo klientin ta instalojë ZemZem si app në telefon ose desktop.</p>
   ...
   <div class="growth-field full"><label>Logo / App Icon</label><input id="pwaIconFile" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml"><input id="pwaIconSource" type="hidden"><div class="growth-actions"><button type="button" class="secondary-btn" id="pwaChooseMedia">Zgjidh nga Media Manager</button><button type="button" class="secondary-btn" id="pwaClearIcon">Pastro ikonën</button></div><small class="muted-note">Rekomandohet logo katrore. Sistemi krijon automatikisht 192×192 dhe 512×512 PNG.</small></div>
   <div class="growth-field full"><div class="growth-actions"><button class="primary-btn">Ruaj PWA</button></div></div></form>
  </article>
  <article class="growth-card"><div class="pwa-preview"><div class="pwa-icon" id="pwaIconPreview">ZZ</div><div><strong id="pwaPreviewName">ZemZem</strong><small style="display:block;opacity:.75;margin-top:4px">Installable Web App</small></div></div><p class="muted-note" style="margin-top:12px">PWA përdor service worker për cache të shell-it bazë dhe manifest dinamik nga konfigurimi i Adminit.</p></article>
 </div></section>`;
...
let pwaIconFile=null,pwaIconSource='';
function renderPwaIcon(url=''){const box=q('#pwaIconPreview');if(!box)return;if(url)box.innerHTML='<img src="'+e(url)+'" alt="App icon">';else box.textContent='ZZ'}
async function sourceToBitmap(source){let blob;if(source instanceof Blob)blob=source;else{const r=await fetch(source,{cache:'no-store'});if(!r.ok)throw new Error('Logoja nuk u lexua.');blob=await r.blob()}return await createImageBitmap(blob)}
async function iconPng(source,size){const img=await sourceToBitmap(source),side=Math.min(img.width,img.height),sx=Math.max(0,(img.width-side)/2),sy=Math.max(0,(img.height-side)/2),canvas=document.createElement('canvas');canvas.width=size;canvas.height=size;const ctx=canvas.getContext('2d');ctx.clearRect(0,0,size,size);ctx.drawImage(img,sx,sy,side,side,0,0,size,size);return await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('Gjenerimi i ikonës dështoi.')),'image/png'))}
async function uploadPwaIcons(source){const stamp=Date.now(),out={};for(const size of [192,512]){const blob=await iconPng(source,size),path='pwa/app-icon-'+size+'-'+stamp+'.png',r=await fetch(SB_URL+'/storage/v1/object/media-library/'+path,{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+session.access_token,'Content-Type':'image/png','x-upsert':'false'},body:blob});if(!r.ok)throw new Error('Upload-i i ikonës '+size+'×'+size+' dështoi.');out[size]=SB_URL+'/storage/v1/object/public/media-library/'+path}return out}
function pickPwaMedia(){if(!window.ZemZemMediaPicker?.open)return t('Media Manager nuk është gati ende.','error');window.ZemZemMediaPicker.open(asset=>{pwaIconFile=null;pwaIconSource=asset.public_url;q('#pwaIconSource').value=pwaIconSource;renderPwaIcon(pwaIconSource);t('Logoja u zgjodh nga Media Manager')})}
async function loadPwa(){try{const rows=await api('site_content?key=eq.pwa&select=content&limit=1');const x=rows?.[0]?.content||{};q('#pwaEnabled').value=String(x.enabled!==false);q('#pwaPrompt').value=String(x.install_prompt_enabled!==false);q('#pwaName').value=x.app_name||'ZemZem';q('#pwaShort').value=x.short_name||'ZemZem';q('#pwaTheme').value=x.theme_color||'#233a4a';q('#pwaBg').value=x.background_color||'#ffffff';q('#pwaDescription').value=x.description||'Libra fizikë dhe eBook nga ZemZem.';q('#pwaPreviewName').textContent=q('#pwaName').value;pwaIconSource=x.icon_source||x.icon_512||x.icon_192||'';q('#pwaIconSource').value=pwaIconSource;renderPwaIcon(x.icon_512||x.icon_192||pwaIconSource)}catch(err){t(err.message,'error')}}
async function savePwa(ev){ev.preventDefault();const btn=ev.submitter||q('#pwaForm button[type="submit"]');if(btn)btn.disabled=true;try{let icon192='',icon512='';const current=await api('site_content?key=eq.pwa&select=content&limit=1'),old=current?.[0]?.content||{};if(pwaIconFile||pwaIconSource){const icons=await uploadPwaIcons(pwaIconFile||pwaIconSource);icon192=icons[192];icon512=icons[512]}else{icon192=old.icon_192||'';icon512=old.icon_512||''}const content={...old,enabled:q('#pwaEnabled').value==='true',install_prompt_enabled:q('#pwaPrompt').value==='true',app_name:q('#pwaName').value.trim()||'ZemZem',short_name:q('#pwaShort').value.trim()||'ZemZem',theme_color:q('#pwaTheme').value,background_color:q('#pwaBg').value,description:q('#pwaDescription').value.trim(),icon_source:pwaIconSource||old.icon_source||'',icon_192:icon192,icon_512:icon512};if(current?.length)await api('site_content?key=eq.pwa',{method:'PATCH',body:{content,updated_at:new Date().toISOString()},prefer:'return=minimal'});else await api('site_content',{method:'POST',body:{key:'pwa',content},prefer:'return=minimal'});pwaIconFile=null;pwaIconSource=content.icon_source||content.icon_512||'';q('#pwaPreviewName').textContent=content.app_name;renderPwaIcon(content.icon_512||content.icon_192);t('PWA dhe logoja e aplikacionit u ruajtën')}catch(err){t(err.message,'error')}finally{if(btn)btn.disabled=false}}
function bind(){
 ...
 q('#pwaForm').addEventListener('submit',savePwa);q('#pwaName').addEventListener('input',()=>q('#pwaPreviewName').textContent=q('#pwaName').value||'ZemZem');q('#pwaIconFile').addEventListener('change',()=>{const f=q('#pwaIconFile').files?.[0];if(!f)return;pwaIconFile=f;pwaIconSource='';q('#pwaIconSource').value='';const u=URL.createObjectURL(f);renderPwaIcon(u)});q('#pwaChooseMedia').onclick=pickPwaMedia;q('#pwaClearIcon').onclick=()=>{pwaIconFile=null;pwaIconSource='';q('#pwaIconSource').value='';q('#pwaIconFile').value='';renderPwaIcon('')};
}
