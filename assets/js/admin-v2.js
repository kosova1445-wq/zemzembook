...
function setView(name){if(!name)return;const permissionName=name==='add-book'?'books':name;if(name==='admins'&&!currentAdminAccess?.is_owner){toast('Vetëm Owner-i mund t’i menaxhojë administratorët.','error');return}if(!hasAdminPermission(permissionName)&&!currentAdminAccess?.full_access){toast('Nuk ke qasje në këtë seksion.','error');return}$$('.nav-item[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===name));$$('.view').forEach(v=>v.classList.toggle('active-view',v.id===`view-${name}`));$('#viewTitle').textContent={dashboard:'Dashboard',orders:'Porositë',books:'Librat fizikë',catalog:'Katalogu',customers:'Klientët',coupons:'Kuponët',reviews:'Review',audit:'Audit Log',integrations:'Integrimet','add-book':'Shto libër'}[name]||name;if(window.innerWidth<=760)$('.sidebar')?.classList.remove('mobile-open')}
