...
async function loadAll(){
  ...
  if(hasAdminPermission('books')||hasAdminPermission('dashboard'))jobs.push(loadBooks(),loadRefs());
...
function resetBookForm(){activeBook=null;ensureBookGalleryFields();$('#bookForm').reset();$('#bookId').value='';$('#bookLanguage').value='sq';$('#bookBadgeText').value='';$('#bookBadgeStyle').value='default';$('#bookOfferEnds').value='';$('#bookReleaseDate').value='';$('#bookCountry').value='';$('#bookDimensions').value='';$('#bookWeight').value='';$('#bookCostPrice').value='';$('#bookStatus').value='draft';$('#bookFormat').value='physical';$('#bookStock').value='0';$('#bookLowStock').value='5';$('#bookTrackStock').checked=true;if($('#bookBackorder'))$('#bookBackorder').checked=false;if($('#bookGiftWrap'))$('#bookGiftWrap').checked=false;if($('#bookGiftWrapPrice'))$('#bookGiftWrapPrice').value='0';if($('#bookPublishAt'))$('#bookPublishAt').value='';if($('#bookUnpublishAt'))$('#bookUnpublishAt').value='';$('#bookCoverUrl').value='';$('#bookSpineCoverUrl').value='';$('#bookBackCoverUrl').value='';$('#bookCoverMode').value='3d';$('#bookThickness').value='';$('#bookSpineCover').value='';$('#bookBackCover').value='';$('#coverPreview').innerHTML='Pa kopertinë';refreshBook3dPreview();renderBookGalleryFields([]);$('#bookModalTitle').textContent='Libër fizik i ri';$('#deleteBookBtn').hidden=true;$('#bookSlug').dataset.manual='';$('#stockHistoryBox')?.remove()}
async function openBook(id,showHistory=false){
  resetBookForm();const b=books.find(x=>x.id===id);
  if(b){activeBook=b;$('#bookId').value=b.id;$('#bookModalTitle').textContent='Edito librin';$('#bookTitle').value=b.title||'';$('#bookSku').value=b.sku||'';$('#bookSlug').value=b.slug||'';$('#bookIsbn').value=b.isbn||'';$('#bookPrice').value=b.price??'';$('#bookOldPrice').value=b.compare_at_price??'';$('#bookBadgeText').value=b.badge_text||'';$('#bookBadgeStyle').value=b.badge_style||'default';$('#bookOfferEnds').value=b.offer_ends_at?new Date(b.offer_ends_at).toISOString().slice(0,16):'';$('#bookStock').value=b.stock_quantity??0;$('#bookLowStock').value=b.low_stock_threshold??5;$('#bookStatus').value=b.status||'draft';$('#bookFormat').value=b.format||'physical';$('#bookLanguage').value=b.language||'sq';$('#bookPages').value=b.pages??'';$('#bookReleaseDate').value=b.release_date||'';$('#bookCountry').value=b.country||'';$('#bookDimensions').value=b.dimensions||'';$('#bookWeight').value=b.weight_grams??'';$('#bookCostPrice').value=b.cost_price??'';$('#bookAuthor').value=b.author_id||'';$('#bookCategory').value=b.category_id||'';$('#bookPublisher').value=b.publisher_id||'';$('#bookShort').value=b.short_description||'';$('#bookDescription').value=b.description||'';$('#bookFeatured').checked=!!b.is_featured;$('#bookBestseller').checked=!!b.is_bestseller;$('#bookPreorder').checked=!!b.is_preorder;if($('#bookBackorder'))$('#bookBackorder').checked=!!b.allow_backorder;if($('#bookGiftWrap'))$('#bookGiftWrap').checked=!!b.gift_wrap_available;if($('#bookGiftWrapPrice'))$('#bookGiftWrapPrice').value=b.gift_wrap_price??0;if($('#bookPublishAt'))$('#bookPublishAt').value=b.publish_at?new Date(new Date(b.publish_at).getTime()-new Date(b.publish_at).getTimezoneOffset()*60000).toISOString().slice(0,16):'';if($('#bookUnpublishAt'))$('#bookUnpublishAt').value=b.unpublish_at?new Date(new Date(b.unpublish_at).getTime()-new Date(b.unpublish_at).getTimezoneOffset()*60000).toISOString().slice(0,16):'';$('#bookTrackStock').checked=b.track_stock!==false;$('#bookCoverUrl').value=b.cover_url||'';$('#bookSpineCoverUrl').value=b.cover_spine_url||'';$('#bookBackCoverUrl').value=b.cover_back_url||'';$('#bookCoverMode').value=b.cover_mode||'flat';$('#bookThickness').value=b.book_thickness_mm??'';$('#coverPreview').innerHTML=b.cover_url?`<img src="${esc(b.cover_url)}" alt="">`:'Pa kopertinë';refreshBook3dPreview();renderBookGalleryFields(b.gallery_urls||[]);$('#deleteBookBtn').hidden=false;await renderStockHistory(b.id,showHistory)}
  mountBookEditorPage();setView('add-book');if(showHistory)setTimeout(()=>$('#stockHistoryBox')?.scrollIntoView({behavior:'smooth',block:'center'}),50);
}
...
function resetCoupon(){const f=$('#couponForm');if(f)f.reset();$('#couponId').value='';$('#couponMin').value='0';$('#couponCustomerLimit').value='';$('#couponFirstOrder').checked=false;if($('#couponRuleType'))$('#couponRuleType').value='standard';if($('#couponBuyQty'))$('#couponBuyQty').value='';if($('#couponGetQty'))$('#couponGetQty').value='';$('#couponScopeType').value='all';couponScopeOptions('all',[]);$('#couponPaypal').checked=true;$('#couponActive').value='true'}
...
function editCoupon(id){const c=coupons.find(x=>x.id===id);if(!c)return;$('#couponId').value=c.id;$('#couponCode').value=c.code;$('#couponType').value=c.discount_type;$('#couponValue').value=c.discount_value;$('#couponMin').value=c.min_order_amount||0;$('#couponMax').value=c.max_discount_amount??'';$('#couponLimit').value=c.usage_limit??'';$('#couponCustomerLimit').value=c.usage_limit_per_customer??'';$('#couponFirstOrder').checked=!!c.first_order_only;if($('#couponRuleType'))$('#couponRuleType').value=c.rule_type||'standard';if($('#couponBuyQty'))$('#couponBuyQty').value=c.buy_quantity??'';if($('#couponGetQty'))$('#couponGetQty').value=c.get_quantity??'';$('#couponScopeType').value=c.scope_type||'all';couponScopeOptions(c.scope_type||'all',c.scope_ids||[]);$('#couponStart').value=c.starts_at?new Date(c.starts_at).toISOString().slice(0,16):'';$('#couponEnd').value=c.ends_at?new Date(c.ends_at).toISOString().slice(0,16):'';$('#couponActive').value=String(c.is_active);$('#couponPaypal').checked=c.allow_with_paypal!==false;$('#couponCode').scrollIntoView({behavior:'smooth',block:'center'})}
async function saveCoupon(e){
  e.preventDefault();if($('#couponRuleType')?.value==='buy_x_get_y'&&(!Number($('#couponBuyQty')?.value)||!Number($('#couponGetQty')?.value))){toast('Për Blej X, merr Y plotëso X dhe Y.','error');return}const id=$('#couponId').value,scopeType=$('#couponScopeType').value||'all',scopeIds=scopeType==='all'?[]:selectedCouponScopeIds();if(scopeType!=='all'&&!scopeIds.length){toast('Zgjidh të paktën një libër ose kategori për kuponin.','error');return}const body={code:$('#couponCode').value.trim().toUpperCase(),discount_type:$('#couponType').value,discount_value:Number($('#couponValue').value),min_order_amount:Number($('#couponMin').value||0),max_discount_amount:$('#couponMax').value?Number($('#couponMax').value):null,usage_limit:$('#couponLimit').value?Number($('#couponLimit').value):null,usage_limit_per_customer:$('#couponCustomerLimit').value?Number($('#couponCustomerLimit').value):null,first_order_only:$('#couponFirstOrder').checked,rule_type:$('#couponRuleType')?.value||'standard',buy_quantity:$('#couponRuleType')?.value==='buy_x_get_y'?Number($('#couponBuyQty')?.value||0)||null:null,get_quantity:$('#couponRuleType')?.value==='buy_x_get_y'?Number($('#couponGetQty')?.value||0)||null:null,scope_type:scopeType,scope_ids:scopeIds,starts_at:$('#couponStart').value?new Date($('#couponStart').value).toISOString():null,ends_at:$('#couponEnd').value?new Date($('#couponEnd').value).toISOString():null,is_active:$('#couponActive').value==='true',allow_with_paypal:$('#couponPaypal').checked,updated_at:new Date().toISOString()};
...
function setView(name){if(!name)return;const permissionName=name==='add-book'?'books':(name==='free-library'?'ebooks':((name==='site-manager'||name==='checkout-settings')?'site':((name==='messages'||name==='team-center')?'dashboard':(name==='business-center'||name==='control-center'?'operations':name))));if(name==='admins'&&!currentAdminAccess?.is_owner){toast('Vetëm Owner-i mund t’i menaxhojë administratorët.','error');return}if(!hasAdminPermission(permissionName)&&!currentAdminAccess?.full_access){toast('Nuk ke qasje në këtë seksion.','error');return}$$('.nav-item[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===name));$$('.view').forEach(v=>v.classList.toggle('active-view',v.id===`view-${name}`));$('#viewTitle').textContent={dashboard:'Dashboard',orders:'Porositë',books:'Librat fizikë',catalog:'Katalogu',customers:'Klientët',coupons:'Kuponët',reviews:'Review',audit:'Audit Log',integrations:'Integrimet','add-book':'Shto libër',messages:'Mesazhet','team-center':'Team Center','free-library':'Biblioteka Falas','site-manager':'Site Manager','checkout-settings':'Checkout & Pages','business-center':'Business Center','control-center':'Control Center'}[name]||name;if(window.innerWidth<=760)$('.sidebar')?.classList.remove('mobile-open')}
function mountBookEditorPage(){
  ...
  card.classList.add('zz-book-editor-page-card');
  $('#bookModal').hidden=true;
}
function bind(){mountBookEditorPage();
  ...
  $('#newBookBtn').onclick=()=>{resetBookForm();mountBookEditorPage();setView('add-book')};$('#bookTitle').addEventListener('input',()=>{if(!$('#bookId').value&&!$('#bookSlug').dataset.manual)$('#bookSlug').value=slugify($('#bookTitle').value)});$('#bookSlug').addEventListener('input',()=>$('#bookSlug').dataset.manual='1');$('#bookForm').addEventListener('submit',saveBook);$('#deleteBookBtn').onclick=deleteBook;$('#bookCover').addEventListener('change',()=>{const f=$('#bookCover').files?.[0];if(f){const u=URL.createObjectURL(f);$('#coverPreview').innerHTML=`<img src="${u}" alt="">`}refreshBook3dPreview()});$('#bookSpineCover')?.addEventListener('change',refreshBook3dPreview);$('#bookBackCover')?.addEventListener('change',refreshBook3dPreview);$('#bookCoverMode')?.addEventListener('change',refreshBook3dPreview);
  $$('[data-close]').forEach(b=>b.onclick=()=>{if(b.dataset.close==='book'){if($('#bookEditorHost .modal-card'))setView('books');else $('#bookModal').hidden=true;}if(b.dataset.close==='order')$('#orderDrawer').hidden=true;if(b.dataset.close==='customer')$('#customerDrawer').hidden=true});$$('[data-add-entity]').forEach(b=>b.onclick=()=>addEntity(b.dataset.addEntity));$('#couponForm').addEventListener('submit',saveCoupon);$('#resetCouponBtn').onclick=resetCoupon;$('#couponScopeType')?.addEventListener('change',e=>couponScopeOptions(e.target.value,[]));$('.mobile-menu-btn').onclick=()=>$('.sidebar')?.classList.toggle('mobile-open');
