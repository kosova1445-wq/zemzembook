...
function resetBookForm(){activeBook=null;ensureBookGalleryFields();$('#bookForm').reset();$('#bookId').value='';$('#bookLanguage').value='sq';$('#bookReleaseDate').value='';$('#bookCountry').value='';$('#bookDimensions').value='';$('#bookWeight').value='';$('#bookStatus').value='draft';$('#bookFormat').value='physical';$('#bookStock').value='0';$('#bookLowStock').value='5';$('#bookTrackStock').checked=true;$('#bookCoverUrl').value='';$('#coverPreview').innerHTML='Pa kopertinë';renderBookGalleryFields([]);$('#bookModalTitle').textContent='Libër fizik i ri';$('#deleteBookBtn').hidden=true;$('#bookSlug').dataset.manual='';$('#stockHistoryBox')?.remove()}
async function openBook(id,showHistory=false){
  resetBookForm();const b=books.find(x=>x.id===id);
  if(b){activeBook=b;$('#bookId').value=b.id;$('#bookModalTitle').textContent='Edito librin';$('#bookTitle').value=b.title||'';$('#bookSku').value=b.sku||'';$('#bookSlug').value=b.slug||'';$('#bookIsbn').value=b.isbn||'';$('#bookPrice').value=b.price??'';$('#bookOldPrice').value=b.compare_at_price??'';$('#bookStock').value=b.stock_quantity??0;$('#bookLowStock').value=b.low_stock_threshold??5;$('#bookStatus').value=b.status||'draft';$('#bookFormat').value=b.format||'physical';$('#bookLanguage').value=b.language||'sq';$('#bookPages').value=b.pages??'';$('#bookReleaseDate').value=b.release_date||'';$('#bookCountry').value=b.country||'';$('#bookDimensions').value=b.dimensions||'';$('#bookWeight').value=b.weight_grams??'';$('#bookAuthor').value=b.author_id||'';$('#bookCategory').value=b.category_id||'';$('#bookPublisher').value=b.publisher_id||'';$('#bookShort').value=b.short_description||'';$('#bookDescription').value=b.description||'';$('#bookFeatured').checked=!!b.is_featured;$('#bookBestseller').checked=!!b.is_bestseller;$('#bookPreorder').checked=!!b.is_preorder;$('#bookTrackStock').checked=b.track_stock!==false;$('#bookCoverUrl').value=b.cover_url||'';$('#coverPreview').innerHTML=b.cover_url?`<img src="${esc(b.cover_url)}" alt="">`:'Pa kopertinë';renderBookGalleryFields(b.gallery_urls||[]);$('#deleteBookBtn').hidden=false;await renderStockHistory(b.id,showHistory)}
...
function resetCoupon(){const f=$('#couponForm');if(f)f.reset();$('#couponId').value='';$('#couponMin').value='0';$('#couponPaypal').checked=true;$('#couponActive').value='true'}
...
async function saveCoupon(e){
  e.preventDefault();const id=$('#couponId').value,body={code:$('#couponCode').value.trim().toUpperCase(),discount_type:$('#couponType').value,discount_value:Number($('#couponValue').value),min_order_amount:Number($('#couponMin').value||0),max_discount_amount:$('#couponMax').value?Number($('#couponMax').value):null,usage_limit:$('#couponLimit').value?Number($('#couponLimit').value):null,starts_at:$('#couponStart').value?new Date($('#couponStart').value).toISOString():null,ends_at:$('#couponEnd').value?new Date($('#couponEnd').value).toISOString():null,is_active:$('#couponActive').value==='true',allow_with_paypal:$('#couponPaypal').checked,updated_at:new Date().toISOString()};
