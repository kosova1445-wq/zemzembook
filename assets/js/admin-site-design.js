(()=>{
  ...
  function applyControls(d){
    ...
    if(q('#siteDesignShelf')){q('#siteDesignShelf').value=Number(d.hero_shelf_opacity??22);q('#siteDesignShelfOut').textContent=q('#siteDesignShelf').value+'%'}
  }
  ...
  async function saveDesign(){
    try{
      const rows=await window.api('site_content?key=eq.homepage&select=content&limit=1');
      const content={...(rows?.[0]?.content||{}),design:collect()};
