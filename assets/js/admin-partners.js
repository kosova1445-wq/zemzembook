...
function bind(){
 ...
 q('#zzSettlementForm')?.addEventListener('submit',async e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const f=e.currentTarget,btn=q('#zzCreateSettlement'),sid=f.supplier_id.value,ps=f.period_start.value,pe=f.period_end.value;const existing=(data?.settlements||[]).find(x=>String(x.supplier_id)===String(sid)&&String(x.period_start).slice(0,10)===ps&&String(x.period_end).slice(0,10)===pe);if(btn){btn.disabled=true;btn.textContent=existing?'Po rifreskohet…':'Po krijohet…'}try{await rpc('admin_partner_create_settlement',{p_supplier_id:sid,p_period_start:ps,p_period_end:pe});q('#zzSettlementDialog').close();await load();const msg=existing?'Barazimi ekzistues u rifreskua me sukses.':'Barazimi u krijua me sukses.';if(window.toast)window.toast(msg);else alert(msg)}catch(err){const raw=String(err?.message||err||'');const msg=/duplicate key|unique constraint|partner_settlements_supplier_id_period_start_period_end_key/i.test(raw)?'Ky barazim për këtë partner dhe periudhë ekziston tashmë.':'Barazimi nuk u ruajt. Provo përsëri.';if(window.toast)window.toast(msg,'error');else alert(msg)}finally{if(btn){btn.disabled=false;btn.textContent='Krijo barazimin'}}});
