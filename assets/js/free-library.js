...
document.addEventListener('DOMContentLoaded',()=>{ensureReportModal();f$('#openSubmit')?.addEventListener('click',openModal);document.querySelectorAll('[data-close-free-library]').forEach(x=>x.addEventListener('click',closeModal));f$('#freeLibraryForm')?.addEventListener('submit',submitBook);f$('#freeLibrarySearch')?.addEventListener('input',renderBooks);f$('#freeLibraryCategory')?.addEventListener('change',renderBooks);loadBooks().catch(()=>{f$('#freeLibraryGrid').innerHTML='<div class="zzfl-empty">Biblioteka nuk mund të ngarkohet për momentin.</div>'})});