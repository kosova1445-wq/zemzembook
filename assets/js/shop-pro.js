(()=>{
  ...
  function languages(){return unique(books().map(b=>b.language||'sq'))}function formats(){return unique(books().map(b=>b.format||'physical'))}function countries(){return unique(books().map(b=>b.country).filter(Boolean))}

  function filterMarkup(){
    const cc=countsFor('cat'), selectedCats=(param('category')||'').split(',').map(decodeURIComponent).filter(Boolean), selectedAuthor=param('author'), selectedLang=param('lang'),selectedFormat=param('format'),selectedCountry=param('country'),selectedPages=param('pages');
    return `<div class="shop-pro-filter-head"><h3>Filtro librat</h3><button class="shop-pro-filter-close" type="button" aria-label="Mbyll filtrat">×</button></div>
      ...
      <div class="filter-group"><strong>Formati</strong><select id="shopProFormat" class="shop-pro-select" style="width:100%"><option value="">Të gjitha formatet</option>${formats().map(v=>`<option value="${escHtml(v)}" ${v===selectedFormat?'selected':''}>${escHtml(v)}</option>`).join('')}</select></div>
      <div class="filter-group"><strong>Shteti / Vendi</strong><select id="shopProCountry" class="shop-pro-select" style="width:100%"><option value="">Të gjitha</option>${countries().map(v=>`<option value="${escHtml(v)}" ${v===selectedCountry?'selected':''}>${escHtml(v)}</option>`).join('')}</select></div>
      <div class="filter-group"><strong>Faqe</strong><select id="shopProPages" class="shop-pro-select" style="width:100%"><option value="">Çdo numër</option><option value="lt200" ${selectedPages==='lt200'?'selected':''}>Nën 200</option><option value="200-400" ${selectedPages==='200-400'?'selected':''}>200–400</option><option value="400-600" ${selectedPages==='400-600'?'selected':''}>400–600</option><option value="600plus" ${selectedPages==='600plus'?'selected':''}>600+</option></select></div>
  ...
  function filtered(){
    const f=getFilters();let items=books().filter(b=>{
      ...
      const pg=Number(b.pages||0),pageOk=!f.pages||(f.pages==='lt200'&&pg>0&&pg<200)||(f.pages==='200-400'&&pg>=200&&pg<400)||(f.pages==='400-600'&&pg>=400&&pg<600)||(f.pages==='600plus'&&pg>=600);return (!f.q||hay.includes(f.q))&&(!f.cats.length||f.cats.includes(b.cat||'Të tjera'))&&(!f.author||b.author===f.author)&&(!f.lang||(b.language||'sq')===f.lang)&&(!f.format||(b.format||'physical')===f.format)&&(!f.country||(b.country||'')===f.country)&&pageOk&&(!f.min||Number(b.price)>=f.min)&&(!f.max||Number(b.price)<=f.max)&&(!f.stock||!b.trackStock||Number(b.stock)>0)&&(!f.offer||Number(b.old)>Number(b.price));
    });
    ...
    try{
      ...
      return `<article class="book-card shop-book-card">
        ...
        <div class="book-meta shop-card-meta">
          ...
          <div class="shop-card-stock ${unavailable?'out':''}">${unavailable?'×':'✓'} ${escHtml(stockText)}</div>${ends?`<div class="offer-countdown" data-offer-countdown="${escHtml(b.offerEndsAt)}"></div>`:''}
  ...
  function render(skipUrl=false){
    ...
    renderChips();renderPagination(pages);applyView();window.ZemZemStore?.syncWishlistButtons?.();window.ZemZemStore?.updateOfferCountdowns?.();if(!skipUrl)syncUrl();
  }
  ...
  function renderChips(){const f=getFilters(),c=$('#shopProChips');if(!c)return;const chips=[];if(f.q)chips.push(['q',`Kërkim: ${$('#shopSearch')?.value||f.q}`]);f.cats.forEach(x=>chips.push(['cat:'+x,x]));if(f.author)chips.push(['author',f.author]);if(f.lang)chips.push(['lang',`Gjuha: ${f.lang.toUpperCase()}`]);if(f.format)chips.push(['format',`Formati: ${f.format}`]);if(f.country)chips.push(['country',`Vendi: ${f.country}`]);if(f.pages)chips.push(['pages',`Faqe: ${f.pages}`]);if(f.min)chips.push(['min',`Nga ${moneyLocal(f.min)}`]);if(f.max)chips.push(['max',`Deri ${moneyLocal(f.max)}`]);if(f.stock)chips.push(['stock','Në stok']);if(f.offer)chips.push(['offer','Në ofertë']);c.hidden=!chips.length;c.innerHTML=chips.map(([k,t])=>`<button type="button" class="shop-pro-chip" data-chip="${escHtml(k)}">${escHtml(t)} ×</button>`).join('')+(chips.length?'<button type="button" class="shop-pro-chip shop-pro-chip-clear" data-chip="all">Pastro të gjitha</button>':'');$$('[data-chip]',c).forEach(b=>b.onclick=()=>clearChip(b.dataset.chip))}
  function clearChip(k){if(k==='all')return clearAll();if(k==='q'&&$('#shopSearch'))$('#shopSearch').value='';else if(k.startsWith('cat:')){const v=k.slice(4);const cb=$$('[data-pro-category]').find(x=>x.dataset.proCategory===v);if(cb)cb.checked=false}else if(k==='author'&&$('#shopProAuthor'))$('#shopProAuthor').value='';else if(k==='lang'&&$('#shopProLanguage'))$('#shopProLanguage').value='';else if(k==='format'&&$('#shopProFormat'))$('#shopProFormat').value='';else if(k==='country'&&$('#shopProCountry'))$('#shopProCountry').value='';else if(k==='pages'&&$('#shopProPages'))$('#shopProPages').value='';else if(k==='min'&&$('#shopProMin'))$('#shopProMin').value='';else if(k==='max'&&$('#shopProMax'))$('#shopProMax').value='';else if(k==='stock'&&$('#shopProStock'))$('#shopProStock').checked=false;else if(k==='offer'&&$('#shopProOffer'))$('#shopProOffer').checked=false;state.page=1;render()}
  function clearAll(){if($('#shopSearch'))$('#shopSearch').value='';$$('[data-pro-category]').forEach(x=>x.checked=false);if($('#shopProAuthor'))$('#shopProAuthor').value='';if($('#shopProLanguage'))$('#shopProLanguage').value='';if($('#shopProFormat'))$('#shopProFormat').value='';if($('#shopProCountry'))$('#shopProCountry').value='';if($('#shopProPages'))$('#shopProPages').value='';if($('#shopProMin'))$('#shopProMin').value='';if($('#shopProMax'))$('#shopProMax').value='';if($('#shopProStock'))$('#shopProStock').checked=false;if($('#shopProOffer'))$('#shopProOffer').checked=false;if($('#sortBooks'))$('#sortBooks').value='new';state.page=1;render()}

  function syncUrl(){const f=getFilters(),p=new URLSearchParams();const rawQ=($('#shopSearch')?.value||'').trim();if(rawQ)p.set('q',rawQ);if(f.cats.length)p.set('category',f.cats.join(','));if(f.author)p.set('author',f.author);if(f.lang)p.set('lang',f.lang);if(f.format)p.set('format',f.format);if(f.country)p.set('country',f.country);if(f.pages)p.set('pages',f.pages);if(f.min)p.set('min',String(f.min));if(f.max)p.set('max',String(f.max));if(f.stock)p.set('stock','1');if(f.offer)p.set('offer','1');if(f.sort!=='new')p.set('sort',f.sort);if(state.perPage!==12)p.set('perPage',String(state.perPage));if(state.page>1)p.set('page',String(state.page));history.replaceState(null,'',location.pathname+(p.toString()?`?${p.toString()}`:''))}
  ...
  function bind(){
    ...
    ['#shopProAuthor','#shopProLanguage','#shopProFormat','#shopProCountry','#shopProPages','#shopProStock','#shopProOffer','#sortBooks'].forEach(s=>$(s)?.addEventListener('change',()=>{state.page=1;render()}));
