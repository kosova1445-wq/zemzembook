...
function orderCard(o){
 ...
 return '<article class="zzpfs-card" data-fulfillment-card="'+o.fulfillment_id+'">'+
  ...
  '<form class="zzpfs-form" data-fulfillment-form="'+o.fulfillment_id+'">'+
    ...
    '<div class="zzpfs-form-foot span-2"><div><small>'+Number(o.item_qty||0)+' artikuj për këtë partner</small>'+(o.shipped_at?'<small>Nisur: '+new Date(o.shipped_at).toLocaleString('sq-AL')+'</small>':'')+(o.delivered_at?'<small>Dorëzuar: '+new Date(o.delivered_at).toLocaleString('sq-AL')+'</small>':'')+'</div>'+(delivered?'<span class="zzpfs-locked">✓ Dorëzimi është final</span>':'<button class="btn primary">Ruaj transportin</button>')+'</div>'+
